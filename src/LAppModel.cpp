...
void LAppModel::Update()
{
    ...
    if (_lipSync)
    {
        if (_lifetimeWavFileHandler) // Check if the pointer is not nullptr
        {
            // リアルタイムでリップシンクを行う場合、システムから音量を取得して0〜1の範囲で値を入力します。
            csmFloat32 value = 0.0f;

            // 状態更新/RMS値取得
            _lifetimeWavFileHandler->Update(deltaTimeSeconds);
            value = _lifetimeWavFileHandler->GetRms(true);
        ...
        {
            // Handle the case where _wavFileHandler is nullptr, perhaps log an error or set a default value
...
void LAppModel::setWavFileHandler(LAppWavFileHandler *handler)
{
    _lifetimeWavFileHandler = handler; // Assign the handler to the model's member variable
}