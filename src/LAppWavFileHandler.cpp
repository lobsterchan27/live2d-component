...
Csm::csmBool LAppWavFileHandler::Update(Csm::csmFloat32 deltaTimeSeconds, float scaleCap)
{
    ...
    // Scale the RMS value if _maxRms is not zero and apply scaling cap
    if (_maxRms != 0) {
        float scaledRms = (rms / _maxRms) * scaleCap;  // Scale and cap the RMS value
        _scaledRms = std::min(scaledRms, scaleCap);    // Ensure it does not exceed the cap
    }
...
void LAppWavFileHandler::ComputeMaxRMS(float deltaTimeSeconds)
{
    ...
    Csm::csmUint32 totalSamples = _wavFileInfo._samplesPerChannel;
    Csm::csmUint32 numUpdates = totalSamples / samplesPerUpdate;

    for (Csm::csmUint32 updateIndex = 0; updateIndex <= numUpdates; updateIndex++)
    {
        Csm::csmUint32 startSample = updateIndex * samplesPerUpdate;
        Csm::csmUint32 endSample = startSample + samplesPerUpdate;
        ...
        float rms = 0.0f;
        for (Csm::csmUint32 channel = 0; channel < _wavFileInfo._numberOfChannels; channel++)
        {
            for (Csm::csmUint32 sample = startSample; sample < endSample; sample++)
            {
                float pcm = _pcmData[channel][sample];
                rms += pcm * pcm;
            }
        }
        rms = sqrt(rms / (_wavFileInfo._numberOfChannels * (endSample - startSample)));
        _maxRms = std::max(_maxRms, rms);
    }
}

Csm::csmFloat32 LAppWavFileHandler::getMaxRMS()
{
    return _maxRms;
}
...
Csm::csmFloat32 LAppWavFileHandler::GetRms(bool scaleRms) const
{
    if (scaleRms) {
        return _scaledRms;
    }
